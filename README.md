...
```
- 2. fstream
  > I think freopen is much better than fstream expecially in algorithms solving. Because when i submit my source code, i dont need to change code in 'freopen' code.<br> freopen을 사용하면 scanf와 cin으로 그냥 받을 수 있기 때문에 freopen을 주로 쓰자
  > 따라서 freopen을 주로 쓰자
...
- 1. segmentation fault
  > segmentation fault란 허용되지 않은 방법으로 메모리에 접근할 때, 허용되지 않은 메모리 영역에 접근할 때 발생해서 사용자가 메모리를 오염시키는 걸 막아주고, 디버깅 하기 힘든 메모리 버그를 알려주는 역할을 합니다.<br>
  > 참조 : <https://hashcode.co.kr/questions/403/segmentation-fault%EB%8A%94-%EB%AD%94%EA%B0%80%EC%9A%94> <br>
